...
trait CassandraCollectionFormats extends CassandraBasicFormats {
  ...
  implicit def listFormat[V](implicit format: CassandraFormat[V], ct: ClassTag[V]) = new CassandraFormat[List[V]] {
    ...
    override def toCql(t: List[V]): String = t.map(v => v.toCql).mkString("[", ", ", "]")

    override def cqlType: String = "list<"+format+">"

    override def read(index: Int, stmt: Gettable): List[V] = stmt.getList(index, ct.runtimeClass).toList.asInstanceOf[List[V]]

    override def read(name: String, stmt: Gettable): List[V] = stmt.getList(name, ct.runtimeClass).toList.asInstanceOf[List[V]]
  ...
  implicit def setFormat[V](implicit format: CassandraFormat[V], ct: ClassTag[V]) = new CassandraFormat[Set[V]] {
    ...
    override def toCql(t: Set[V]): String = t.map(v => v.toCql).mkString("{", ", ", "}")

    override def cqlType: String = "set<"+format.cqlType+">"

    override def read(index: Int, stmt: Gettable): Set[V] = stmt.getSet(index, ct.runtimeClass).toSet.asInstanceOf[Set[V]]

    override def read(name: String, stmt: Gettable): Set[V] = stmt.getSet(name, ct.runtimeClass).toSet.asInstanceOf[Set[V]]
  ...
  implicit def mapFormat[K, V](implicit kformat: CassandraFormat[K], vformat: CassandraFormat[V], ctk: ClassTag[K], ctv: ClassTag[V]) = new CassandraFormat[Map[K, V]] {
    ...
    override def toCql(t: Map[K, V]): String = t.map(e => e._1.toCql+": "+e._2.toCql).mkString("{", ", ", "}")

    override def cqlType: String = "map<"+kformat.cqlType+", "+vformat.cqlType+">"

    override def read(index: Int, stmt: Gettable): Map[K, V] = stmt.getMap(index, ctk.runtimeClass, ctv.runtimeClass).toMap.asInstanceOf[Map[K,V]]

    override def read(name: String, stmt: Gettable): Map[K, V] = stmt.getMap(name, ctk.runtimeClass, ctv.runtimeClass).toMap.asInstanceOf[Map[K,V]]
